...
const callCommand = async (command: Interaction) => {
  try {
    ...
    await updateUser(member.user.id, member.user.name, member.user.avatar, prisma)
    ...
    const configCommand: Command | undefined = commands[data.name] ? commands[data.name] : undefined
    console.log(data.name)
    if (!configCommand) throw new Error(`Command ${data.name} not found`)
    ...
    if (userPerm.opts.maxCalls && calls.length >= userPerm.opts.maxCalls) throw new Error(`You have reached the max calls of ${userPermission.opts.maxCalls} for your permission level`)
    ...
    if (configCommand.type === 'info' && !data.resolved?.members) {
      ...
      let infoBlock: InfoBlock = {}
      infoBlock = {
        name: permission.opts.info?.includes('name') ? member.user.username : '',
        rep: permission.opts.info?.includes('rep') ? callerRep.rep : undefined,
        rank: permission.opts.info?.includes('rank') ? ranks?.find(r => r.minRep <= callerRep.rep)?.name : '',
        pos: permission.ops.info?.includes('pos') ? await (await prisma.rep.findMany({ where: { serverid: guild_id }, orderBy: { rep: 'desc' } })).findIndex(rep => rep.userid === callerRep.userid) + 1 : undefined
      }
      await prisma.$disconnect()
      return `${infoBlock.name}'s rep is ${infoBlock.rep} (${infoBlock.rank}), ${infoBlock.pos}${infoBlock.pos == 1 ? 'st' : infoBlock.pos == 2 ? 'nd' : infoBlock.pos == 3 ? 'rd' : 'th'} in the server`
    }
    ...
    if (userPerm.allowedOn) {
      ...
      switch(configCommand.permType) {
        case 'rank':
          const targetRank = ranks?.find(r => r.minRep <= targetRep.rep)
          if (!targetRank) throw new Error('Target user does not have a rank')
          if (!userPermission.allowedOn.includes(targetRank.name)) throw new Error(`Cannot call ${data.name} on ${targetRank.name}`)
              break
        case 'role':
          const targetRole = roles?.find(r => targetUser.roles?.includes(r.roleid))
          if (!targetRole) throw new Error('Target user does not have a rank')
          if (!userPerm.allowedOn.contains(userPerm.name)) throw new Error(`Cannot call ${data.name} on ${targetRole.name}`)
          break
        case 'all':
          break
      }
    ...
    switch(configCommand.type) {
      case 'adjust':
        ...
        returnMessage = `${targetUser.user.username}'s rep has been adjusted by ${permission?.options.amount ?? 0} (${data.name})`
        break
      case 'set': case 'ban':
        if (!targetRep || !target || !targetUser.user) throw new Error(`No target user provided for command ${data.name}`)
        const amount = permission.opts.amount != null && configCommand.type === 'ban' ? { name: 'amount', value: permission.opts.amount.toString() } : data?.options?.find((option: Option) => option.name === 'amount')
        if (!amount) throw new Error('No amount provided')
        if (configCommand.type === 'ban') {
          if (parseInt(amount.value) < 0) {
            // unban the person
